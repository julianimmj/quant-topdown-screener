...
def render_ranking_table(
    ...
    # Construção de tabela HTML pura com zebra striping perfeito e suporte a scroll
    rows_html = []
    for i, (_, row) in enumerate(display_df.iterrows()):
        # Alternância nítida de tons escuros institucionais
        bg_color = "#121622" if i % 2 == 0 else "#191F2F"
        ...
        if "Compra Forte" in status_str:
            status_badge = '<span style="background: rgba(0, 212, 170, 0.15); color: #00D4AA; border: 1px solid #00D4AA; padding: 3px 8px; border-radius: 12px; font-weight: 700; font-size: 0.82rem; white-space: nowrap;">🟢 Compra Forte</span>'
        elif "Compra" in status_str:
            status_badge = '<span style="background: rgba(255, 215, 0, 0.15); color: #FFD700; border: 1px solid #FFD700; padding: 3px 8px; border-radius: 12px; font-weight: 700; font-size: 0.82rem; white-space: nowrap;">🟡 Compra</span>'
        elif "Sobrecomprado" in status_str:
            status_badge = '<span style="background: rgba(255, 68, 68, 0.15); color: #FF4444; border: 1px solid #FF4444; padding: 3px 8px; border-radius: 12px; font-weight: 700; font-size: 0.82rem; white-space: nowrap;">🔴 Sobrecomprado</span>'
        elif "Pullback" in status_str:
            status_badge = '<span style="background: rgba(255, 140, 0, 0.15); color: #FF8C00; border: 1px solid #FF8C00; padding: 3px 8px; border-radius: 12px; font-weight: 700; font-size: 0.82rem; white-space: nowrap;">🟠 Aguardar Pullback</span>'
        else:
            status_badge = f'<span style="background: rgba(255, 255, 255, 0.06); color: #BBB; border: 1px solid #444; padding: 3px 8px; border-radius: 12px; font-size: 0.82rem; white-space: nowrap;">{status_str}</span>'

        # Helper para colorir números de sub-score
        def num_color(val):
            try:
                v = float(val)
                if v >= 65: return f'<span style="color: #00D4AA; font-weight: 600;">{v:.0f}</span>'
        ...
        row_html = f"""
        <tr style="background-color: {bg_color}; {left_border} transition: background-color 0.15s ease;" onmouseover="this.style.backgroundColor='#232B3E'" onmouseout="this.style.backgroundColor='{bg_color}'">
            <td style="padding: 10px 8px; text-align: center;">{rank_html}</td>
            <td style="padding: 10px 10px; font-weight: 700; color: #FFFFFF;"><b style="color: #00D4AA;">{row.get('Ticker', '')}</b></td>
            <td style="padding: 10px 10px; color: #CCD6F6; font-size: 0.88rem;">{row.get('Setor', '')}</td>
            <td style="padding: 10px 10px; color: #E0E6ED; font-weight: 600; text-align: right;">{price_val}</td>
            <td style="padding: 10px 10px; text-align: center;">
                <span style="background: rgba(0, 212, 170, 0.12); color: {tqs_col}; border: 1px solid {tqs_col}80; padding: 3px 10px; border-radius: 6px; font-weight: 800; font-size: 0.95rem;">
                    {tqs:.1f}
                </span>
            </td>
            <td style="padding: 10px 8px; text-align: center;">{num_color(row.get('Macro', 0))}</td>
            <td style="padding: 10px 8px; text-align: center;">{num_color(row.get('Setor RS', 0))}</td>
            <td style="padding: 10px 8px; text-align: center;">{num_color(row.get('Trend', 0))}</td>
            <td style="padding: 10px 8px; text-align: center;">{num_color(row.get('Trigger', 0))}</td>
            <td style="padding: 10px 8px; text-align: right; color: #CCD6F6;">{adx_val}</td>
            <td style="padding: 10px 8px; text-align: right; color: #CCD6F6;">{er_val}</td>
            <td style="padding: 10px 8px; text-align: right; color: #CCD6F6;">{rsi_val}</td>
            <td style="padding: 10px 10px; text-align: center;">{status_badge}</td>
        </tr>
        """
...
def render_score_decomposition(score: FinalScore) -> None:
    ...
    for i, (label, val) in enumerate(trend_items.items()):
        ...
        with cols[i]:
            st.markdown(
                ...
                <span style="color: {color}; font-size: 1.25em; font-weight: 700;">
                {val:.1f}</span></div>""",
                unsafe_allow_html=True,
            )
    ...
    if score.trigger:
        ...
        with row1_c1:
            st.markdown(
                f"""<div style="background: #151922; border: 1px solid #2A303C; border-radius: 8px; padding: 10px; text-align: center; margin-bottom: 8px;">
        ...
        with row2_c1:
            st.markdown(
                f"""<div style="background: #151922; border: 1px solid #2A303C; border-radius: 8px; padding: 10px; text-align: center;">
        ...
        with row2_c2:
            st.markdown(
                f"""<div style="background: {status_bg}; border: 1px solid {status_color}; border-radius: 8px; padding: 10px; text-align: center;">
